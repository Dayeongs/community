...
%>
						<!-- a태그는 클릭하면 새로운페이지를 요청한다. (reflash)
						     ajax를 사용하기 위해서 a태그 이벤트의 기본동작을 막아야 한다. 
						     event를 추가하여 preventDefault()메소드를 사용한다. -->
						<a href="" class="list-group-item list-group-item-action"
							onclick="getEmployees(<%=dept.getId() %>, event)"><%=dept.getName() %></a>
<%
	}
%>
					</div>
	 ...
	 xhr.onreadystatechange = function() {				// 이벤트 핸들러 등록
		 if (xhr.readyState == 4 && xhr.status == 200) {
			/* 
				jsonText
			 		[
			 			{"id":100, "firstName":"홍", "lastName":"길동", "hireDate":"2021-01-01", "jobId":"IT_PROG","salary":4000.0},
			 			{"id":100, "firstName":"홍", "lastName":"길동", "hireDate":"2021-01-01", "jobId":"IT_PROG","salary":4000.0},
			 			...
			 		]
			*/
