...
<%
	/*
		요청 URL
			localhost/community/board/delete.jsp?no=xxx&page=xxx
		요청 파라미터
			no=xxx&page=xxx
	*/
	...
	if (loginUser == null) {
		response.sendRedirect("/community/board/delete.jsp?error=deny");
		return;
	}
	...
	// 5. 변경된 내용이 포함된 Board객체를 전달해서 updateBoard(Board board) 메소드를 실행시켜
	//    테이블에 변경된 내용이 반영되게 하기
